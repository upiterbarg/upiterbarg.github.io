...
class MixUp(nn.Module):
    ...
    def forward(self, *x):
        assert len(x) == len(self.mix_types), "{} but {}".format(
            len(x), len(self.mix_types)
        )
        if self.training and random.uniform(0, 1) < self.mix_probability:
            ...
            for sub_x, mix_type in zip(x, self.mix_types):
                if sub_x is None:
                    ...
                    if mix_type == "sum":
                        y.append(lam * padded_sub_x_a + (1.0 - lam) * padded_sub_x_b)
                    elif mix_type == "max":
                        y.append(
                            torch.stack([padded_sub_x_a, padded_sub_x_b], dim=0).max(
                                dim=0
                            )[0]
                        )
                    else:
                        raise NotImplementedError(
                            "Unknown mix_type: {}".format(mix_type)
                        )
            return y
        return [*x]
