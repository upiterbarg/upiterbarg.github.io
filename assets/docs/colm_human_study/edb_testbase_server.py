            ...
            while True:
                try:
                    ...
                    if try_no == 3:
                        ...
                        self.loop.run_until_complete(self.con.execute("ROLLBACK;"))
                        try_no += 1
                else:
                    break
    ...
    def __new__(mcls, name, bases, ns):
        ...
        if not ns.get("BASE_TEST_CLASS") and hasattr(cls, "get_database_name"):
            ...
            if name in mcls._database_names:
                raise TypeError(f"{name} wants duplicate database name: {dbname}")
...
def _init_cluster(data_dir=None, *, cleanup_atexit=True, init_settings=None):
    if init_settings is None:
        init_settings = {}
    if not os.environ.get("EDGEDB_DEBUG_SERVER") and not os.environ.get(
        "EDGEDB_LOG_LEVEL"
    ):
    ...
    async def assert_query_result(
        ...
        *,
        ...
        try:
            ...
            await tx.start()
            try:
                res = await self.con.fetchall_json(query, *fetch_args, **fetch_kw)
            finally:
                await tx.rollback()
    ...
    def _assert_data_shape(self, data, shape, message=None):
        ...
        def _assert_list_shape(path, data, shape):
            if not isinstance(data, list):
                self.fail(f"{message}: expected list " f"{_format_path(path)}")

            if not data and shape:
                self.fail(
                    f"{message}: expected non-empty list " f"{_format_path(path)}"
                )
            ...
            for _data_count, el in enumerate(data):
                ...
                _assert_generic_shape(path + (f"[{_data_count}]",), el, el_shape)
        ...
        def _assert_set_shape(path, data, shape):
            if not isinstance(data, (list, set)):
                self.fail(f"{message}: expected list or set " f"{_format_path(path)}")

            if not data and shape:
                self.fail(f"{message}: expected non-empty set " f"{_format_path(path)}")
        ...
        def _assert_generic_shape(path, data, shape):
            ...
            if isinstance(shape, list):
                ...
                if data != str(shape):
                    self.fail(
                        f"{message}: {data!r} != {shape!r} " f"{_format_path(path)}"
                    )
...
class CLITestCaseMixin:
    def run_cli(self, *args, input=None):
        ...
        return runner.invoke(
            cli.cli, args=cmd_args, input=input, catch_exceptions=False
        )
    ...
    def setUp(self):
        if self.INTERNAL_TESTMODE:
            self.loop.run_until_complete(
                self.con.execute("CONFIGURE SESSION SET __internal_testmode := true;")
            )
        ...
        if self.SETUP_METHOD:
            self.loop.run_until_complete(self.con.execute(self.SETUP_METHOD))
    ...
    def tearDown(self):
        try:
            if self.TEARDOWN_METHOD:
                self.loop.run_until_complete(self.con.execute(self.TEARDOWN_METHOD))
        finally:
            try:
                if self.ISOLATED_METHODS:
                ...
                if self.con.is_in_transaction():
                    ...
                    raise AssertionError(
                        "test connection is still in transaction " "*after* the test"
                    )

                if not self.ISOLATED_METHODS:
                    self.loop.run_until_complete(self.con.execute("RESET ALIAS *;"))
            ...
            try:
                ...
                try:
                    if cls.admin_conn is not None:
                        cls.loop.run_until_complete(cls.admin_conn.aclose())
                finally:
                    super().tearDownClass()
    ...
    async def _run():
        if num_jobs == 1:
            ...
            async with taskgroup.TaskGroup(name="setup test cases") as g:
                ...
                for _case, dbname, setup_script in setup:
                    g.create_task(
                        controller(_setup_database, dbname, setup_script, conn)
                    )
...
async def _setup_database(dbname, setup_script, conn_args):
    ...
    admin_conn = await edgedb.async_connect(
        database=edgedb_defines.EDGEDB_SUPERUSER_DB, **default_args
    )
